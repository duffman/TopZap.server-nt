...
export class BarcodeParser {
	 ...
	 * @returns {string}
	 */
	public static prepZeroToLength(input: string, length: number): string {
		let result: string = "";
	 ...
	 * @returns {string}
	 */
	public static prepEan13Code(input: string, strict: boolean = false): string {
		if (strict && !( input.match(/^[0-9]+$/) != null)) {
			return PStrUtils.fillChar("0", 13);
		} else {
			return BarcodeParser.prepZeroToLength(input, 13);
		}
	}

	public validateEan(eanCode: string): boolean {
		...
		let originalCheck: string = eanCode.substring(eanCode.length - 1);
		...
		// Divide total by 10 and set the remainder
