...
export class ExpressMysqlSession {
	db: DbManager;
	...
	DoSomething(req: Request, res: Response, next: NextFunction): void {
	}
...
module.exports = function(session) {
	...
	var MySQLStore = function(options, conn, cb) {
		...
		if (!this.conn) {
			this.conn = mysql.createPool(this.options);
		}
	...
	MySQLStore.prototype.defaultOptions = {
		...
		// Number of connections when creating a conn pool:
		connectionLimit: 1,
		// Whether or not to end the database conn when the store is closed:
	...
	MySQLStore.prototype.setOptions = function(options) {
		...
		this.options = _.defaults({}, options || {}, {
			// The default value of this option depends on whether or not a conn was passed to the constructor.
			endConnectionOnClose: !this.conn,
		}, this.defaultOptions);
	...
	MySQLStore.prototype.query = function(sql, params, cb) {
		...
		var done = _.once(cb);
		var promise = this.conn.query(sql, params, done);
	...
	MySQLStore.prototype.close = function(cb) {
		...
		if (this.conn && this.options.endConnectionOnClose) {
			var promise = this.conn.end(done);
			if (promise && _.isFunction(promise.then) && _.isFunction(promise.catch)) {
				// Probably a promise.
