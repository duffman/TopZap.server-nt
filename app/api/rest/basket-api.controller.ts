...
import { ISessionPullResult }     from '@zapModels/messages/session-pull-result';
...
import {BasketChannelController} from '@app/controllers/basket.channel-controller';
...
export class BasketApiController implements IApiController {
	...
	basketChannel: BasketChannelController;

	constructor(public debugMode: boolean = false) {
		this.searchService = new PriceSearchService(null);

		this.basketChannel = new BasketChannelController()

		this.basketChannel.onChannelData(data => {
			console.log("BasketChannelController :: DATA ::", data);
		});

		/*
		const drone = new Scaledrone('Ian6hNLkyvwxagXL');
	...
	public initRoutes(routes: Router): void  {
		routes.get("/balle", (req: Request, resp: Response) => {
			...
			this.basketChannel.emitGetOffersInit("kalle", {balle: "kuling"});

			console.log("BALLE CALLED ::", req.session);
			console.log("BALLE CALLED ::", req.sessionID);
